...
type Choice struct {
	...
	Message      MessageOai `json:"message"`
	FinishReason *string    `json:"finish_reason"`
}

type ChunkChoice struct {
	Index        int        `json:"index"`
	Delta        MessageOai `json:"delta"`
	FinishReason *string    `json:"finish_reason"`
}
...
type ChatCompletionRequest struct {
	...
	Messages         []MessageOai    `json:"messages"`
...
func toToolCalls(tc []ToolCall) []ToolCall {
	toolCalls := make([]ToolCall, len(tc))
	for i, tc := range tc {
		toolCalls[i].ID = toolCallId()
		toolCalls[i].Type = "function"
		toolCalls[i].Function.Name = tc.Function.Name
		toolCalls[i].Index = 0 //tc.Function.Index
...
func toChatCompletion(id string, r ChatResponse) ChatCompletion {
	...
	return ChatCompletion{
		...
		Choices: []Choice{{
			Index:   0,
			Message: MessageOai{Role: r.Message.Role, Content: r.Message.Content, ToolCalls: toolCalls},
...
func toChunk(id string, r ChatResponse, toolCallSent bool) ChatCompletionChunk {
	...
	return ChatCompletionChunk{
		...
		Choices: []ChunkChoice{{
			Index: 0,
			Delta: MessageOai{Role: "assistant", Content: r.Message.Content, ToolCalls: toolCalls},
			FinishReason: func(reason string) *string {
				if len(reason) > 0 {
					if toolCallSent {
...
func fromChatRequest(r ChatCompletionRequest) (*ChatRequest, error) {
	...
	for _, msg := range r.Messages {
		...
		case []any:
			for _, c := range content {
				...
				case "image_url":
					...
					messages = append(messages, Message{Role: msg.Role, Images: []ImageData{img}})
				default:
					return nil, errors.New("invalid message format")
				}
			}
		default:
			...
			messages = append(messages, Message{Role: msg.Role, ToolCalls: toolCalls})
		}
	}
